...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '') {
        ...
        if (!$up['success']) {
            ...
            if ($rid > 0) {
                db_exec('UPDATE facilities SET title = ?, title_en = ?, icon = ?, description = ?, description_en = ?, image = ?, sort = ?, active = ? WHERE id = ?',
                    [$title, $titleEn, $icon, $description, $descriptionEn, $up['file'], $sort, $active, $rid]);
                $targetId = $rid;
                flash('success', 'Fasilitas berhasil diperbarui.');
            } else {
                db_exec('INSERT INTO facilities (title, title_en, icon, description, description_en, image, sort, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ...
      <div class="card-body">
        <form method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" name="title" id="facTitle" class="form-control" required value="<?= e($edit['title'] ?? '') ?>">
          </div>
          <div class="form-group">
            <label>Nama Fasilitas (EN - terjemahan Inggris)</label>
            <div class="input-group">
              <input type="text" name="title_en" id="facTitleEn" class="form-control" value="<?= e($edit['title_en'] ?? '') ?>">
              <div class="input-group-append">
                <button type="button" class="btn btn-outline-accent js-auto-translate" data-src="#facTitle" data-target="#facTitleEn" data-lang="en" title="Terjemahkan otomatis ke Inggris"><i class="fas fa-language mr-1"></i>EN</button>
              </div>
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <textarea name="description" id="facDesc" class="form-control" rows="3"><?= e($edit['description'] ?? '') ?></textarea>
          </div>
          <div class="form-group">
            <label>Deskripsi (EN)</label>
            <div class="input-group">
              <textarea name="description_en" id="facDescEn" class="form-control" rows="3"><?= e($edit['description_en'] ?? '') ?></textarea>
              <div class="input-group-append">
                <button type="button" class="btn btn-outline-accent js-auto-translate" data-src="#facDesc" data-target="#facDescEn" data-lang="en" title="Terjemahkan otomatis ke Inggris"><i class="fas fa-language mr-1"></i>EN</button>
              </div>
            </div>
          </div>
          <div class="form-group">
