...
$id = (int)($_GET['id'] ?? 0);
$post = db_one("SELECT p.*, c.name AS category_name, c.name_en AS category_name_en, c.id AS category_id FROM posts p LEFT JOIN categories c ON c.id = p.category_id WHERE p.id = ? AND p.status = 'published' AND p.published_at <= NOW()", [$id]);

if (!$post) {
    redirect(base_url('news.php'));
}

$pageTitle = pick($post, 'title');
$active = 'news';

$recent = db_all("SELECT id, title, title_en, published_at FROM posts WHERE status = 'published' AND published_at <= NOW() AND id <> ? ORDER BY published_at DESC LIMIT 5", [$id]);
  ...
  <div class="container">
    <div class="row g-5">
      <div class="col-lg-8" data-aos="fade-up">
        ...
        <h1 class="mb-4" style="font-size:clamp(1.6rem,3vw,2.3rem)"><?= e(pick($post, 'title')) ?></h1>
        <div class="rounded-4 overflow-hidden mb-4">
          <img src="<?= e($pImg) ?>" alt="<?= e(pick($post, 'title')) ?>" style="width:100%;max-height:420px;object-fit:cover">
        </div>
        <?php if ($post['excerpt']): ?>
        <div class="lead fw-semibold mb-4" style="color:var(--primary)"><?= e(pick($post, 'excerpt')) ?></div>
        <?php endif; ?>
      ...
      <div class="col-lg-4">
        <div class="sticky-top" style="top:120px" data-aos="fade-left">
          <div class="p-4 rounded-4 border surface-card mb-4" style="border-color:var(--border)!important">
            ...
            <?php if ($recent): ?>
              ...
              <a href="<?= e(base_url('news-detail.php?id=' . $r['id'])) ?>" class="d-block py-2 border-bottom" style="border-color:var(--border-soft)!important">
                <span class="fw-semibold d-block" style="color:var(--ink)"><?= e(pick($r, 'title')) ?></span>
          ...
          <div class="p-4 rounded-4 border surface-card" style="border-color:var(--border)!important">
            ...
            <a href="<?= e(base_url('news.php?category=' . $cat['id'])) ?>" class="chip text-decoration-none mb-2 me-1 d-inline-block"><?= e(pick($cat, 'name')) ?></a>
            <?php endforeach; ?>
          </div>
        </div>
      </div>
